 ...
 *
 * Sends a channel NOTICE message to <nickname> on <channel> that bypasses flood protection limits. The target nickname
 * must be in the same channel as the client issuing the command, and the client must be a channel operator.
 *
 ...
 * RPL_ISUPPORT reply (numeric 005) with the CNOTICE keyword
 */
final class Cnotice extends Command
{
    /**
     * @var string
     */
    ...
    {
        return "CNOTICE <nickname> <channel> :<message>";
    }
    ...
    {
        return "CNOTICE {$this->getNickName()} {$this->getChannel()} :{$this->getMessage()}";
    }
} 
